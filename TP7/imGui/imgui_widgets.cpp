...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    {
        // Vertical separator, for drawMenu bars (use current line height). Not exposed because it is misleading and it doesn't have an effect on regular layout.
...
// With typical options: Left-click on color square to open color picker. Right-click to open option drawMenu. CTRL-Click over input fields to edit them and TAB to go to next item.
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // Context drawMenu: display and modify options (before defaults are applied)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Context drawMenu: display and store options.
    if (!(flags & ImGuiColorEditFlags_NoOptions))
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // We use NoHoldingActiveID on menus so user can click and _hold_ on a drawMenu then drag to browse child entries
...
// Currently the main responsibility of this function being to setup clip-rect + horizontal layout + drawMenu navigation layer.
// Ideally we also want this to be responsible for claiming space out of the main window scrolling rectangle, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
// Then later the same system could be used for multiple drawMenu-bars, scrollbars, side-bars.
...
void ImGui::EndMenuBar()
{
    ...
    // Nav: When a move request within one of our child drawMenu failed, capture the request to navigate among our siblings.
...
bool ImGui::BeginMainMenuBar()
{
    ...
    // For the main drawMenu bar, which cannot be moved, we honor g.Style.DisplaySafeAreaPadding to ensure text can be visible on a TV set.
    ...
    if (menu_bar_window == NULL || menu_bar_window->BeginCount == 0)
    {
        ...
        // However drawMenu-bar will affect the minimum window size so we'll get the right height.
    ...
    PushStyleVar(ImGuiStyleVar_WindowMinSize, ImVec2(0, 0)); // Lift normal size constraint, however the presence of a drawMenu-bar will give us the minimum height we want.
...
void ImGui::EndMainMenuBar()
{
    ...
    // When the user has left the drawMenu layer (typically: closed menus through activation of an item), we restore focus to the previous window
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    // Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu would steal focus and not allow hovering on parent drawMenu)
    ImGuiWindowFlags flags = ImGuiWindowFlags_ChildMenu | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoNavFocus;
    if (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu))
        flags |= ImGuiWindowFlags_ChildWindow;

    // If a drawMenu with same the ID was already submitted, we will append to it, matching the behavior of Begin().
    ...
    // Tag drawMenu as used. Next time BeginMenu() with same ID is called it will append to existing menu
    ...
    if (menuset_is_open)
        g.NavWindow = window;  // Odd hack to allow hovering across menus of a same drawMenu-set (otherwise we wouldn't be able to hover parent)

    // The reference position stored in popup_pos will be used by Begin() to find a suitable position for the child drawMenu,
    // However the final position is going to be different! It is chosen by FindBestWindowPosForPopup().
    // e.g. Menus tend to overlap each other horizontally to amplify relative Z-ordering.
    ImVec2 popup_pos, pos = window->DC.CursorPos;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Menu inside an horizontal drawMenu bar
    ...
    {
        // Menu inside a drawMenu
        // (In a typical drawMenu window where all items are BeginMenu() or MenuItem() calls, extra_w will always be 0.0f.
    ...
    {
        // Close drawMenu when not hovering it anymore unless we are moving roughly in the direction of the menu
    ...
    {
        // Menu bar
        if (menu_is_open && pressed && menuset_is_open) // Click an open drawMenu again to close it
        {
            want_close = true;
    ...
    if (!enabled) // explicitly close if an open drawMenu becomes disabled, facilitate users code a lot in pattern such as 'if (BeginMenu("options", has_object)) { ..use object.. }'
        want_close = true;
    ...
    {
        // Don't recycle same menu level in the same frame, first close the other drawMenu and yield for a frame.
        OpenPopup(label);
        return false;
    }
...
void ImGui::EndMenu()
{
    // Nav: When a left move request _within our child menu_ failed, close ourselves (the _parent_ drawMenu).
    // A drawMenu doesn't close itself because EndMenuBar() wants the catch the last Left<>Right inputs.
    // However, it means that with the current code, a BeginMenu() from outside another drawMenu or a menu-bar won't be closable with the Left direction.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Mimic the exact layout spacing of BeginMenu() to allow MenuItem() inside a drawMenu bar, which is a little misleading but may be useful
    ...
    {
        // Menu item inside a vertical drawMenu
        // (In a typical drawMenu window where all items are BeginMenu() or MenuItem() calls, extra_w will always be 0.0f.
        //  Only when they are other items sticking out we're going to add spacing, yet only register minimum width into the layout system.
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // We make a call to ItemAdd() so that attempts to use a contextual popup drawMenu with an implicit ID won't use an older ID.
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
    if (p_open && !*p_open)
    {
    ...
    // Select with right mouse button. This is so the common idiom for context drawMenu automatically highlight the current widget.
