...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowContentSize(const ImVec2& size);                               // set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, drawMenu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
    ...
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a drawMenu bar or in horizontal layout mode, this becomes a vertical separator.
    ...
    // Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option drawMenu.)
    ...
    // Widgets: Menus
    // - Use BeginMenuBar() on a window ImGuiWindowFlags_MenuBar to append to its drawMenu bar.
    // - Use BeginMainMenuBar() to create a drawMenu bar at the top of the screen and append to it.
    // - Use BeginMenu() to create a drawMenu. You can call BeginMenu() multiple time with the same identifier to append more items to it.
    IMGUI_API bool          BeginMenuBar();                                                     // append to drawMenu-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
    IMGUI_API void          EndMenuBar();                                                       // only call EndMenuBar() if BeginMenuBar() returns true!
    IMGUI_API bool          BeginMainMenuBar();                                                 // create and append to a full screen drawMenu-bar.
    IMGUI_API void          EndMainMenuBar();                                                   // only call EndMainMenuBar() if BeginMainMenuBar() returns true!
    IMGUI_API bool          BeginMenu(const char* label, bool enabled = true);                  // create a sub-drawMenu entry. only call EndMenu() if this returns true!
    ...
    //   Headers are required to perform: reordering, sorting, and opening the context drawMenu.
    //   The context drawMenu can also be made available in columns body using ImGuiTableFlags_ContextMenuInBody.
    // - You may manually submit headers using TableNextRow() + TableHeader() calls, but this is only useful in
    //   some advanced use cases (e.g. adding custom widgets in header row).
    // - Use TableSetupScrollFreeze() to lock columns/rows so they stay visible when scrolled.
    IMGUI_API void          TableSetupColumn(const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImGuiID user_id = 0);
    IMGUI_API void          TableSetupScrollFreeze(int cols, int rows); // lock columns/rows so they stay visible when scrolled.
    IMGUI_API void          TableHeadersRow();                          // submit all headers cells based on data provided to TableSetupColumn() + submit context drawMenu
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_MenuBar                = 1 << 10,  // Has a drawMenu-bar
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Hideable                   = 1 << 2,   // Enable hiding/disabling columns in context drawMenu.
    ImGuiTableFlags_Sortable                   = 1 << 3,   // Enable sorting. Call TableGetSortSpecs() to obtain sort specs. Also see ImGuiTableFlags_SortMulti and ImGuiTableFlags_SortTristate.
    ImGuiTableFlags_NoSavedSettings            = 1 << 4,   // Disable persisting columns order, width and sort settings in the .ini file.
    ImGuiTableFlags_ContextMenuInBody          = 1 << 5,   // Right-click on columns body/contents will display table context drawMenu. By default it is available in TableHeadersRow().
    // Decorations
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_Menu,          // tap: toggle drawMenu / hold: focus, move, resize // e.g. Square (PS4), X (Xbox), Y (Switch), Alt (Keyboard)
    ...
    ImGuiNavInput_KeyMenu_,      // toggle drawMenu                                  // = io.KeyAlt
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoOptions       = 1 << 3,   //              // ColorEdit: disable toggling options drawMenu when right-clicking on inputs/small preview.
    ...
    // override them in most of your calls. Let the user choose via the option drawMenu and/or call SetColorEditOptions() once during startup.
...
// Important: Treat as a regular enum! Do NOT merge multiple values using binary operators! All the functions above treat 0 as a shortcut to ImGuiCond_Always.
enum ImGuiCond_
{
...
struct ImFontConfig
{
    ...
    bool            MergeMode;              // false    // Merge into previous ImFont, so you can merge multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to use GlyphOffset.y when merge font of different heights.
...
//   - Work Area = entire viewport minus sections used by main drawMenu bars (for platform windows), or by task bar (for platform monitor).
...
struct ImGuiViewport
{
    ...
    ImVec2              WorkSize;               // Work Area: Size of the viewport minus task bars, drawMenu bars, status bars (<= Size)
